...
class HandTracker:
    ...
    def __init__(self,
        ...
        # Use new MediaPipe Tasks API for version 0.10+
        ...
        # Drawing utilities (try legacy, fallback to manual)
        try:
            self.mp_drawing = mp.solutions.drawing_utils
            self.mp_drawing_styles = mp.solutions.drawing_styles
            self.has_legacy_drawing = True
        except AttributeError:
            self.has_legacy_drawing = False

        # Initialize Hand Landmarker
        options = self.hand_landmarker_options(
            base_options=self.base_options(model_asset_path='hand_landmarker.task'),
            running_mode=self.vision_running_mode.IMAGE if static_image_mode else self.vision_running_mode.VIDEO,
        ...
        try:
            ...
            print(f"❌ Hand landmarker initialization failed: {str(e)}")
            raise RuntimeError(f"MediaPipe Hand Landmarker initialization failed: {str(e)}")
    ...
    def _convert_to_legacy_format(self, results):
        """Convert new API results to legacy format for compatibility"""
        try:
            # Create a simple object to mimic legacy multi_hand_landmarks
            ...
            for hand_landmarks in results.hand_landmarks:
                legacy_hand = LegacyLandmarks(hand_landmarks)
                legacy_results.append(legacy_hand)
    ...
    def draw_landmarks(self, frame: np.ndarray, hand_landmarks_list: List) -> np.ndarray:
        ...
        try:
            ...
            if self.has_legacy_drawing and self.use_legacy:
                # Legacy API drawing
                ...
                # New API or manual drawing
                for hand_landmarks in hand_landmarks_list:
                    # Draw connections manually
                    self._draw_hand_connections(annotated_frame, hand_landmarks.landmark)
    ...
    def _draw_hand_connections(self, frame: np.ndarray, landmarks):
        """Draw hand landmark connections manually for new API"""
        try:
            height, width = frame.shape[:2]

            # Define hand connections (same as legacy HAND_CONNECTIONS)
            connections = [
