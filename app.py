...
CNN_MODEL_PATH = 'models/trained_models/cnn_gesture_model.h5'  # Update with actual path
LSTM_MODEL_PATH = None  # Set to path if you have a trained LSTM model
...
if 'gesture_recognizer' not in st.session_state:
    st.session_state.gesture_recognizer = GestureRecognizer(
        use_cnn=True,
        use_lstm=False,  # Set to True if you have LSTM model
...
if 'speech_processor' not in st.session_state:
    st.session_state.speech_processor = SpeechProcessor(use_gtts=True)
...
def main():
    ...
    with st.sidebar:
        ...
        # Auto speak toggle
        auto_speak = st.checkbox("🔊 Auto Speak Translations", value=st.session_state.auto_speak)
        ...
        if st.session_state.current_mode == "Speech to Sign":
            ...
            if st.button("🎙️ Listen for Speech", disabled=st.session_state.is_listening):
                st.session_state.is_listening = True
                with st.spinner("Listening for speech..."):
                    speech_text = st.session_state.speech_processor.speech_to_text(timeout=10.0)
                    if speech_text:
                        ...
                        st.error("No speech detected or recognition failed")
                st.session_state.is_listening = False
    ...
    with col1:
        ...
        if camera_enabled and st.session_state.current_mode == "Sign to Speech":
            ...
            st.info("Toggle 'Auto Speak' in sidebar for automatic audio playback after each gesture sequence.")
            ...
            if st.session_state.translation_output:
                ...
                st.info("Visual hand positions for your spoken words.")
    ...
    try:
        ...
        if hand_landmarks:
            ...
            if features is not None:
                ...
                static_prediction = st.session_state.gesture_recognizer.recognize_static_gesture(
                    features
                )
                ...
                if static_prediction:
                    ...
                    if st.session_state.current_mode == "Sign to Speech":
                        ...
                        if st.session_state.auto_speak and sentence:
                            st.session_state.speech_processor.text_to_speech(sentence)
                            st.success("🔊 Translation spoken automatically")
